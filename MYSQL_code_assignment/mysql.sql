...
SELECT id AS videoID, title FROM video
WHERE video.id NOT IN (SELECT sourceVideoID FROM annotation);

SELECT videoID, username, ratingTime AS ratingTimestamp
FROM rating INNER JOIN user ON user.id = rating.linkedUser
-- no ties
WHERE ratingTime = (SELECT MAX(ratingTime) FROM rating);

SELECT videoID, title
FROM content_creator INNER JOIN cocreator ON content_creator.id = cocreator.creatorID INNER JOIN video ON cocreator.videoID = video.id
WHERE content_creator.screenName LIKE 'TaylorSwiftOfficial' AND video.viewCount >= 1000000;

SELECT id AS videoID, title, COUNT(destinationVideoID) AS linkedCount
FROM video
LEFT JOIN annotation ON id = destinationVideoID
GROUP BY videoID, title
-- can not use limit 1 due to there might be ties
HAVING COUNT(destinationVideoID) = (
    SELECT COUNT(destinationVideoID) AS maxLinkedCount
    FROM annotation
    GROUP BY destinationVideoID
    ORDER BY maxLinkedCount DESC
    LIMIT 1
)
-- may not be required
ORDER BY videoID;

SELECT rating.videoID, uploaded AS uploadDatetime, COUNT(linkedUser) AS ratingCount
FROM video INNER JOIN rating ON video.id = rating.videoID
INNER JOIN video_hashtag ON video_hashtag.videoID = video.id
INNER JOIN hashtag ON hashtagID = hashtag.id
WHERE tag LIKE '#memes'
...
FROM content_creator INNER JOIN user ON content_creator.id = user.id
WHERE reputation < 50 
AND content_creator.id IN (SELECT creatorID 
FROM cocreator INNER JOIN content_creator ON cocreator.creatorID = content_creator.id
GROUP BY cocreator.creatorID
HAVING COUNT(cocreator.videoID) >= 3)
AND content_creator.id IN (SELECT cocreator.creatorID
FROM video INNER JOIN cocreator ON video.id = cocreator.videoID
INNER JOIN rating ON rating.videoID = video.id
GROUP BY cocreator.creatorID
HAVING COUNT(rating.videoID) >= 6);

SELECT tag AS hashtag, COUNT(comment) AS commentCount
FROM hashtag INNER JOIN video_hashtag ON hashtag.id = video_hashtag.hashtagID
INNER JOIN video ON video_hashtag.videoID = video.id
INNER JOIN rating ON video.id = rating.videoID
WHERE comment like '%thank you%' OR comment LIKE '%well done%'
GROUP BY hashtag.id
-- return all ties
HAVING commentCount = (SELECT COUNT(comment) AS commentCount
FROM hashtag INNER JOIN video_hashtag ON hashtag.id = video_hashtag.hashtagID
INNER JOIN video ON video_hashtag.videoID = video.id
INNER JOIN rating ON video.id = rating.videoID
where comment like '%thank you%' OR comment LIKE '%well done%'
...
SELECT tag AS hashtag, COUNT(sourceVideoID) AS totalAnnotationsAsDestination, SUM(duration) AS totalDuration
FROM hashtag INNER JOIN video_hashtag ON hashtag.id = video_hashtag.hashtagID
INNER JOIN video ON video.id= video_hashtag.videoID
INNER JOIN annotation ON video.id = destinationVideoID
GROUP BY hashtag.id
HAVING totalAnnotationsAsDestination IN (SELECT sourceCount FROM
-- write in this form due to cannot use limit directly inside of in()
(SELECT DISTINCT COUNT(sourceVideoID) AS sourceCount
FROM hashtag INNER JOIN video_hashtag ON hashtag.id = video_hashtag.hashtagID
INNER JOIN video ON video.id= video_hashtag.videoID
INNER JOIN annotation ON video.id = destinationVideoID
GROUP BY hashtag.id
ORDER BY COUNT(sourceVideoID) DESC
LIMIT 3) AS topThreeCount)
-- the annotation counts in examples in the question8 are listed in desc order
ORDER BY totalAnnotationsAsDestination DESC;

SELECT realName,screenName 
FROM content_creator
WHERE content_creator.id IN (SELECT cocreator.creatorID
FROM cocreator
WHERE cocreator.videoID IN (SELECT cocreator.videoID 
FROM cocreator
WHERE cocreator.creatorID IN (SELECT content_creator_hashtag.creatorID  
FROM content_creator_hashtag 
INNER JOIN hashtag ON content_creator_hashtag.hashtagID = hashtag.id
WHERE hashtag.tag LIKE '#memes'))
AND cocreator.creatorID IN (SELECT content_creator_hashtag.creatorID  
FROM content_creator_hashtag 
INNER JOIN hashtag ON content_creator_hashtag.hashtagID = hashtag.id
WHERE hashtag.tag LIKE '#technology'))
OR content_creator.id IN (SELECT cocreator.creatorID
FROM cocreator
WHERE cocreator.videoID IN (SELECT cocreator.videoID 
FROM cocreator
WHERE cocreator.creatorID IN (SELECT content_creator_hashtag.creatorID  
...
WHERE content_creator.id IN(SELECT cocreator.creatorID
FROM cocreator
WHERE cocreator.videoID NOT IN (SELECT cocreator.videoID
FROM cocreator 
INNER JOIN video ON cocreator.videoID = video.id
WHERE uploaded < '2023-01-01'
AND cocreator.videoID IN (cocreator.videoID IN (SELECT cocreator.videoID
FROM content_creator INNER JOIN cocreator ON content_creator.id= cocreator.creatorID
WHERE content_creator.screenName LIKE 'INFO20003Memes')))
AND cocreator.videoID IN (SELECT cocreator.videoID
FROM cocreator 
INNER JOIN video ON cocreator.videoID = video.id
WHERE uploaded > '2023-01-01'
AND cocreator.videoID IN (cocreator.videoID IN (SELECT cocreator.videoID
